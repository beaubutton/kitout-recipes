...
@test "mission-control-defaults: step validates" {
  run kitout validate -m "$MANIFEST"
  [ "$status" -eq 0 ]
}

@test "mission-control-defaults: apply sets the keys and is idempotent" {
  require_apply
  run kitout apply -y -m "$MANIFEST"
  [ "$status" -eq 0 ]
  [ "$(defaults read com.apple.dock mru-spaces)" = "0" ]
  [ "$(defaults read com.apple.dock expose-group-apps)" = "1" ]
