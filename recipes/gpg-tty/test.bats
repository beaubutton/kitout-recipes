...
@test "gpg-tty: step validates" {
  run kitout validate -m "$MANIFEST"
  [ "$status" -eq 0 ]
}

@test "gpg-tty: writes the block and is idempotent (isolated HOME)" {
  require_apply
  export HOME="$WORK/home"; mkdir -p "$HOME"; printf '# my zshrc\n' >"$HOME/.zshrc"

  run kitout apply -y -m "$MANIFEST"
  [ "$status" -eq 0 ]
  grep -q '>>> recipes:gpg-tty' "$HOME/.zshrc"
  grep -q 'GPG_TTY=$(tty)' "$HOME/.zshrc"
