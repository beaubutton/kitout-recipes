...
@test "scroll-bars-always: step validates" {
  run kitout validate -m "$MANIFEST"
  [ "$status" -eq 0 ]
}

@test "scroll-bars-always: apply sets the key and is idempotent" {
  require_apply
  run kitout apply -y -m "$MANIFEST"
  [ "$status" -eq 0 ]
  [ "$(defaults read NSGlobalDomain AppleShowScrollBars)" = "Always" ]
