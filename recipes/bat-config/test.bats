...
@test "bat-config: step validates" {
  run kitout validate -m "$MANIFEST"
  [ "$status" -eq 0 ]
}

@test "bat-config: writes the block and is idempotent (isolated HOME)" {
  require_apply
  export HOME="$WORK/home"; mkdir -p "$HOME"; printf '# my zshrc\n' >"$HOME/.zshrc"

  run kitout apply -y -m "$MANIFEST"
  [ "$status" -eq 0 ]
  grep -q '>>> recipes:bat-config' "$HOME/.zshrc"
  grep -q 'BAT_THEME="ansi"' "$HOME/.zshrc"
  grep -q 'MANPAGER=' "$HOME/.zshrc"
