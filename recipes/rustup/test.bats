...
@test "rustup: step validates" {
  run kitout validate -m "$MANIFEST"
  [ "$status" -eq 0 ]
}

@test "rustup: probe convergence — present binary is satisfied, absent is pending (CI-safe)" {
...
@test "rustup: apply installs rustup and is idempotent (VM only)" {
  require_apply
  command -v brew >/dev/null || skip "brew not on PATH"
  command -v rustup >/dev/null && skip "rustup already installed — refusing to re-test on a real machine"
