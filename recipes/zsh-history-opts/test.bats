...
@test "zsh-history-opts: step validates" {
  run kitout validate -m "$MANIFEST"
  [ "$status" -eq 0 ]
}

@test "zsh-history-opts: writes the block and is idempotent (isolated HOME)" {
  require_apply
  export HOME="$WORK/home"; mkdir -p "$HOME"; printf '# my zshrc\n' >"$HOME/.zshrc"

  run kitout apply -y -m "$MANIFEST"
  [ "$status" -eq 0 ]
  grep -q '>>> recipes:zsh-history-opts' "$HOME/.zshrc"
  grep -q 'HISTSIZE=100000' "$HOME/.zshrc"
  grep -q 'SHARE_HISTORY' "$HOME/.zshrc"
