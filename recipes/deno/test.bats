...
@test "deno: step validates" {
  run kitout validate -m "$MANIFEST"
  [ "$status" -eq 0 ]
}

@test "deno: probe convergence — present binary is satisfied, absent is pending (CI-safe)" {
...
@test "deno: apply installs deno and is idempotent (VM only)" {
  require_apply
  command -v brew >/dev/null || skip "brew not on PATH"
  command -v deno >/dev/null && skip "deno already installed — refusing to re-test on a real machine"
