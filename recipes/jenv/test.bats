...
@test "jenv: step validates" {
  run kitout validate -m "$MANIFEST"
  [ "$status" -eq 0 ]
}

@test "jenv: writes the activation block and is idempotent (isolated HOME)" {
  require_apply
  export HOME="$WORK/home"; mkdir -p "$HOME"; printf '# my zshrc\n' >"$HOME/.zshrc"

  run kitout apply -y -m "$MANIFEST"
  [ "$status" -eq 0 ]
  grep -q '>>> recipes:jenv' "$HOME/.zshrc"
  grep -qF '.jenv/bin:$PATH' "$HOME/.zshrc"
  grep -q 'jenv init -' "$HOME/.zshrc"
